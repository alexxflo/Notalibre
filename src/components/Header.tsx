...
import { Gem, Users, Shield, LayoutDashboard, Camera } from 'lucide-react';
      ...
      <div className="container mx-auto flex justify-between items-center p-4 gap-2">
        ...
        <div className="flex-none">
            ...
            {isUserLoading ? (
                ...
                <Button variant="ghost" size="sm" onClick={() => setView('flog')} className="text-yellow-400 hover:bg-yellow-900/50 hover:text-yellow-300 uppercase shrink-0 p-2 md:px-3">
                  <Camera className="h-4 w-4 md:mr-2"/>
                  <span className="hidden md:inline">Mi Flog</span>
                </Button>
                {user?.uid === ADMIN_UID && (
